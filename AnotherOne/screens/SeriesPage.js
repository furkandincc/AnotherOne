...
const SeriesPage = () => {
  ...
  const [series, setSeriess] = useState([]); // Filmleri sakla
  const [loading, setLoading] = useState(true); // Yüklenme durumu
  const navigation = useNavigation(); 

  useEffect(() => {
    // Python dosyasını çalıştır
    fetch('http://192.168.1.170:3000/run-python/series')
      .then((response) => response.json())
      .then((data) => {
        console.log('Gelen veri:', data);
        setSeriess(data); // Alınan veriyi state'e kaydet
        setLoading(false);
      })
      .catch((error) => {
...
const filteredSeriess = Array.isArray(series)
  ? series.filter((series) => series.name.toLowerCase().includes(searchQuery.toLowerCase()))
  : [];
  ...
  return (
    <View style={styles.container}>
      ...
      {/* Filmler Listesi */}
      <FlatList
        data={filteredSeriess} // Filtrelenmiş müzikler
        keyExtractor={(item) => item.ID.toString()}
        renderItem={({ item }) => (
          <View style={styles.card}>
            <Text style={styles.cardTitle}>{item.name}</Text>
            <Text style={styles.cardDescription}>{item.description}</Text>
            <Text style={styles.cardEpisode}>Episodes: {item.epi}</Text>
