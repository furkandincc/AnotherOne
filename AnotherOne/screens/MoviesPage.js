...
const MoviesPage = () => {
  ...
  const [movies, setMovies] = useState([]); // Filmleri sakla
  const [loading, setLoading] = useState(true); // Yüklenme durumu
  const navigation = useNavigation(); 

  useEffect(() => {
    // Python dosyasını çalıştır ve output.json verilerini al
    fetch('http://192.168.1.170:3000/run-python/film')
      .then((response) => response.json())
      .then((data) => {
        console.log('Gelen veri:', data);
        setMovies(data); // Alınan veriyi state'e kaydet
        setLoading(false);
      })
      .catch((error) => {
...
const filteredMovies = Array.isArray(movies)
  ? movies.filter((film) => film.name.toLowerCase().includes(searchQuery.toLowerCase()))
  : [];
